...
class ReportCert(Base):
    __tablename__ = 'ReportCerts'
    
    id = Column(Integer, primary_key=True, index=True)
    FK_Report_id = Column(Integer, ForeignKey('Reports.id'), nullable=False)
    certname = Column(String(256), nullable=False)
    hostname = Column(String(256), nullable=False)
    obtain_date = Column(Date, nullable=False)
