...
CREATE TABLE `Users`(
	`uuid` UUID NOT NULL 
		COMMENT 'Auto-generated Unique User ID' 
		DEFAULT UUID(),
	...
	`passwordSalt` VARCHAR(128) NOT NULL
		COMMENT 'Salt used to hash the password',
...
CREATE TABLE `UsersExperience`(
	...
	`isCurrent` BOOLEAN NOT NULL 
		COMMENT 'is employee still working there',
	`beginDate` DATE NOT NULL 
		COMMENT 'Work''s date of beginning',
	`endDate` DATE 
		COMMENT 'Work''s date of end',
...
CREATE TABLE `UsersEducation`(
	...
	`FK_reportId` INT UNSIGNED NOT NULL,
	`level`
		ENUM('primary', 'secondary', 'higher', 'bachelor', 'master', 'doctorate', 'vocational', 'continuing')
		NOT NULL 
		COMMENT 'Education level',
	`school` VARCHAR(255) NOT NULL 
		COMMENT 'Finished school name',
	`specialization` VARCHAR(80) 
		COMMENT 'Specialization',
	`isCurrent` BOOLEAN NOT NULL 
		COMMENT 'is user still studying there',
	`beginDate` DATE NOT NULL 
		COMMENT 'Education''s date of beginning',
	`endDate` DATE 
		COMMENT 'Education''s date of end',
...
CREATE TABLE `Courses`(
	...
	`places` INT NOT NULL 
		COMMENT 'Available places',
	`isOnline` BOOLEAN NOT NULL 
		COMMENT 'is it course online',
...
CREATE TABLE `JobOffers`(
	...
	`name` VARCHAR(255) NOT NULL 
		COMMENT 'Job''s name',
	`isOnline` BOOLEAN NOT NULL 
		COMMENT 'is it remote job',
	
	PRIMARY KEY(`id`)
) COMMENT 'Companies'' job offers';

CREATE TABLE `Companies`(
	`nip` VARCHAR(13) NOT NULL 
		COMMENT 'Company''s NIP number',
	`FK_categoryId` INT UNSIGNED,
	`name` VARCHAR(80) NOT NULL 
		COMMENT 'Company''s name',
	`email` VARCHAR(80) NOT NULL 
		COMMENT 'Company''s e-mail address',
	`passwordHash` VARCHAR(255) NOT NULL 
		COMMENT 'Hash of company''s password',
	`phoneNumber` VARCHAR(20) 
...
CREATE USER 'hyacinthus'@'%';
SET PASSWORD FOR 'hyacinthus'@'%' = PASSWORD('FlowerChamaedorea000Z');	
